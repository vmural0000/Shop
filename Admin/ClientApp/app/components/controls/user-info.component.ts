...
import { UserEdit } from '../users/user-edit.model';
...
export class UserInfoComponent implements OnInit {
    ...
    private onCurrentUserDataLoadFailed(error: any) {
        this.alertService.error(`Unable to retrieve user data from the server.\r\nErrors: "${Utilities.getHttpResponseMessage(error)}"`, error);
    ...
    private saveSuccessHelper(user?: User) {
        ...
        if (this.isGeneralEditor) {
            if (this.isNewUser)
                this.alertService.success( `User \"${this.user.userName}\" was created successfully`);
            else if (!this.isEditingSelf)
                this.alertService.success(`Changes to user \"${this.user.userName}\" was saved successfully`);
        }

        if (this.isEditingSelf) {
            this.alertService.success("Changes to your User Profile was saved successfully");
    ...
    private saveFailedHelper(error: any) {
        this.isSaving = false;
        this.alertService.error("The below errors occured whilst saving your changes:", error);
    ...
    private cancel() {
        ...
        this.alertService.info("Operation cancelled by user");
    ...
    private refreshLoggedInUser() {
        this.accountService.refreshLoggedInUser()
            ...
            error => {
                this.alertService.error("An error occured whilst refreshing logged in user information from the server", error);
            });
    }
    ...
    private unlockUser() {
        ...
        this.accountService.unblockUser(this.userEdit.id)
            ...
            error => {
                this.isSaving = false;
                this.alertService.error("The below errors occured whilst unblocking the user:", error);
            });
    }
